...
public abstract aspect AbstractDurationSimpleMethodValueParameters {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParameters.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParameters.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("AbstractDurationSimpleMethodsValueParameters fichier trace = "
						+ fileTrace);
		String params = "0";
		
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParameters.parameters")) {
			params = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParameters.parameters");
		}
		...
		for (int i = 0; i < len; i++) {
			titleParams = titleParams + "param_" + tabParams[i]+"_value" + sep+"param_" + tabParams[i]+"_length"+sep;
		}
		System.out.println("Title = "+"####time"+sep+"class.method"+sep+ titleParams
				+ "time called method in millisecondes(ms)\n");
		outDurationMethods = new Trace("####time"+sep+"class.method"+sep+ titleParams
				+ "time called method in millisecondes(ms)\n", fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParameters.durationMini"));
	}
	...
	Object around(): methods()	 {
		...
		if (duree/1000000 >= durationMini) {
			...
			Object[] tabArgs = thisJoinPoint.getArgs();
			...
			for (int i = 0; i < len; i++) {
				if (null != tabArgs[tabParams[i]].toString()) {
					...
					//valArgs = valArgs + tabArgs[tabParams[i]].toString().replaceAll("\n", " ") + ";";
					valArgs = new StringBuilder(valArgs ).append(tabArgs[tabParams[i]].toString()).append(sep)
					.append(tabArgs[tabParams[i]].toString().length()).append(sep).toString();
				} else {
			...
			outDurationMethods.append(new StringBuilder(outDurationMethods
					...
					sep).append(valArgs).append(df.format(((double)duree/1000000))).append("\n")
					.toString());
