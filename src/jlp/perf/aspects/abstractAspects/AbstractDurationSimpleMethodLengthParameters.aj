...
public abstract aspect AbstractDurationSimpleMethodLengthParameters {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParameters.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParameters.filelogs");
		} else {
		...
		String params = "0";
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParameters.parameters")) {
			params = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParameters.parameters");
		}
		...
		for (int i = 0; i < len; i++) {
			titleParams = titleParams + "lenghtparam_" + tabParams[i] + sep;
		}
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParameters.durationMini"));
	}
		...
		if (duree/1000000 >= durationMini) {
			...
			for (int i = 0; i < len; i++) {
				if (null != tabArgs[tabParams[i]].toString()) {
					...
					longArgs = longArgs + Integer.toString(tabArgs[tabParams[i]].toString().length()) + sep;
					
				} else {
					longArgs = longArgs + "0"+sep;
				}
			}
			int  noLine=0;
			...
			outDurationMethods.append(new StringBuilder(outDurationMethods
					...
					.append(
					sep).append(longArgs).append(df.format(((double)duree/1000000))).append("\n")
					.toString());
