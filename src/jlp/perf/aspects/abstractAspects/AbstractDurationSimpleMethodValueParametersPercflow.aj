...
public abstract aspect AbstractDurationSimpleMethodValueParametersPercflow percflow(methods()){
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParametersPercflow.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParametersPercflow.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("AbstractDurationSimpleMethodsValueParametersPercflow fichier trace = "
									+ fileTrace);
		String params ="0";
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParametersPercflow.parameters")) {
			params = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParametersPercflow.parameters");
		} 
		if(params.indexOf(",")>=0)
		{
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueParametersPercflow.durationMini"));
	}
		...
		if (duree/1000000 >= durationMini) {
			...
			for (int i = 0; i < len; i++) {
				if (null != tabArgs[tabParams[i]].toString()) {
					...
					valArgs = new StringBuilder(valArgs).append("no toString()").append(sep).toString();
				}
			}
			int  noLine=0;
