...
public abstract aspect AbstractDurationSimpleMethodWithThreads {
	...
	static {
		...
		if (props.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodWithThreads.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodWithThreads.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

    System.out.println("AbstractDurationSimpleMethodsWithTreads fichier trace = "+fileTrace);
		outDurationMethods = new Trace("####time"+sep+"Class.methods"+sep+"Thread_Name"+sep+"time in millisecondes(ms)\n",fileTrace);
		/*outDurationMethods
		...
		durationMini = Double.parseDouble(props
				.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodWithThreads.durationMini"));
	}
			...
			outDurationMethods
					.append(new StringBuilder(outDurationMethods.getSdf()
							...
							.append(".").append(
								...
								.append(sep).append(Thread.currentThread().getName())	
							.append(sep).append(
				...
				.append(new StringBuilder(outDurationMethods.getSdf()
						...
						.append(
								thisJoinPoint.getSignature()
										.getDeclaringTypeName())
						.append(".").append(
								thisJoinPoint.getSignature().getName()).append("_line").
								append(noLine)
							.append(sep).append(Thread.currentThread().getName())	
						.append(sep).append(
								df.format(((double)duree/1000000))).append("\n").toString());
