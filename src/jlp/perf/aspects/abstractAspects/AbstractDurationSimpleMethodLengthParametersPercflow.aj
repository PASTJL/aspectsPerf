...
public abstract aspect AbstractDurationSimpleMethodLengthParametersPercflow percflow(methods()){
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParametersPercflow.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParametersPercflow.filelogs");
		} else {
		...
		String params ="0";
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParametersPercflow.parameters")) {
			params = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParametersPercflow.parameters");
		} 
		if(params.indexOf(",")>=0)
		{
		...
		for (int i=0;i<len;i++)
			{
			titleParams=titleParams+"lengthparam_"+tabParams[i]+sep;
			}
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthParametersPercflow.durationMini"));
	}
		...
		if (duree/1000000 >= durationMini) {
			...
			for (int i = 0; i < len; i++) {
				if (null != tabArgs[tabParams[i]].toString()) {
					...
					longArgs = new StringBuilder(longArgs).append( Integer.toString(tabArgs[tabParams[i]].toString().length())).append(sep).toString();
				} else {
					longArgs = new StringBuilder(longArgs).append("0").append(sep).toString();
				}
			}
			int noLine=0;
