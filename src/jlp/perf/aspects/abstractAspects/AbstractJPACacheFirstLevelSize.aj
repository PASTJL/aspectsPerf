...
public privileged abstract aspect AbstractJPACacheFirstLevelSize {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractJPACacheFirstLevelSize.filelogs")) {
			fileTrace = dirLogs
					+ props.getProperty("jlp.perf.aspects.abstractAspects.AbstractJPACacheFirstLevelSize.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("Dans static AbstractJPACacheFirstLevelSize type trace : abstractJPACacheFirstLevelSize fichier trace = "
						+ fileTrace);
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractJPACacheFirstLevelSize.frequenceMeasure")) {
			frequenceMeasure = Long
					.parseLong(props
							.getProperty("jlp.perf.aspects.abstractAspects.AbstractJPACacheFirstLevelSize.frequenceMeasure"));
		}

		serialization = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.abstractAspects.AbstractJPACacheFirstLevelSize.serialization",
								"true"));
...
//		 ||
//		execution( public 	* javax.persistence.EntityManager+.close())
//		|| execution( public 	* javax.persistence.EntityManager+.is*());

	before(): methods() 
	{
		// System.out.println("Rentree dans before");

		synchronized (AbstractJPACacheFirstLevelSize.this) {
