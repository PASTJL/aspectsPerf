...
public abstract aspect AbstractDurationSimpleMethodPercflow percflow(methods()) {
	...
	static {
		...
		if (props.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodPercflow.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodPercflow.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

    System.out.println("AbstractDurationSimpleMethod14 fichier trace = "+fileTrace);
		outDurationMethods = new Trace("####time"+sep+"Class.methods"+sep+"time in millisecondes(ms)\n",fileTrace);
		/*outDurationMethods
		...
		durationMini = Double.parseDouble(props
				.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodPercflow.durationMini"));
	}
