...
public abstract aspect AbstractGenericAspect3 {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.perf.aspects.abstractAspects.AbstractGenericAspect3");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect3.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect3.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect3.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.abstractAspects.AbstractGenericAspect3.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect3.depthIntrospect",
						"All:-1;").split(";");
	...
	before(): methods()
	{
		...
		if (isTimeFreq) {
			Long lastTime = AbstractGenericAspect3.hmCurrentAge.get(nameClass);
			if ((System.currentTimeMillis() - lastTime) > (long) frequenceMeasure) {
				AbstractGenericAspect3.hmCurrentAge.put(nameClass,System.currentTimeMillis());
			} else {
				boolExec = false;
			}
			...
			if (lastTime >= (long) frequenceMeasure) {
				AbstractGenericAspect3.hmCurrentAge.put(nameClass,1L);
			} else {
				AbstractGenericAspect3.hmCurrentAge.put(nameClass,lastTime + 1);
				boolExec = false;
			}
		}
		...
		if (boolExec) {
			...
			Trace outFile=AbstractGenericAspect3.hmOutFiles.get(nameClass);
			if (hmBean.size()> 20)
				{
				hmBean.clear();
				counter=0;
