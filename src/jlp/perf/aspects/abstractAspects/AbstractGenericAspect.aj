...
public abstract aspect AbstractGenericAspect {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.perf.aspects.abstractAspects.AbstractGenericAspect");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.abstractAspects.AbstractGenericAspect.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect.depthIntrospect",
						"All:-1;").split(";");
	...
	before(): methods()
	{
		...
		if (isTimeFreq) {
			Long lastTime = AbstractGenericAspect.hmCurrentAge.get(nameClass);
			if ((System.currentTimeMillis() - lastTime) > (long) frequenceMeasure) {
				AbstractGenericAspect.hmCurrentAge.put(nameClass,System.currentTimeMillis());
			} else {
				boolExec = false;
			}
			...
			if (lastTime >= (long) frequenceMeasure) {
				AbstractGenericAspect.hmCurrentAge.put(nameClass,1L);
			} else {
				AbstractGenericAspect.hmCurrentAge.put(nameClass,lastTime + 1);
				boolExec = false;
			}
		}
		...
		if (boolExec) {
			...
			Trace outFile=AbstractGenericAspect.hmOutFiles.get(nameClass);
			if (hmBean.size()> 20)
				{
				hmBean.clear();
				counter=0;
