...
public abstract aspect AbstractDurationSimpleMethodLenghtParametersPercflow percflow(methods()){
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLenghtParametersPercflow.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLenghtParametersPercflow.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("AbstractDurationSimpleMethodsLengthParametersPercflow fichier trace = "
									+ fileTrace);
		String params ="0";
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLenghtParametersPercflow.parameters")) {
			params = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLenghtParametersPercflow.parameters");
		} 
		if(params.indexOf(",")>=0)
		{
		...
		for (int i=0;i<len;i++)
			{
			titleParams=titleParams+"lengthparam_"+tabParams[i]+";";
			}
		
		outDurationMethods = new Trace(
				"####time;class.method;"+titleParams+"time called method in millisecondes(ms)\n",
				fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLenghtParametersPercflow.durationMini"));
	}
	...
	after (): methods()	 {
		...
		if (duree/1000000 >= AbstractDurationSimpleMethodLenghtParametersPercflow.durationMini) {
			...
			for (int i = 0; i < len; i++) {
				if (null != tabArgs[tabParams[i]].toString()) {
					...
					longArgs = longArgs + Integer.toString(tabArgs[tabParams[i]].toString().length()) + ";";
				} else {
					longArgs = longArgs + "0;";
				}
			}
			...
			outDurationMethods.append(new StringBuilder(
					...
					.append(";").append(
							longArgs).append(df.format(((double)duree/1000000)))
					.append("\n").toString());
			}
			else
			{
