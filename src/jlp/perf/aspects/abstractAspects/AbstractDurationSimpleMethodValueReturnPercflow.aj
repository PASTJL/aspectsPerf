...
public abstract aspect AbstractDurationSimpleMethodValueReturnPercflow percflow(methods()){
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueReturnPercflow.filelogs")) {
			fileTrace =dirLogs+ props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueReturnPercflow.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("AbstractDurationSimpleMethodsValueReturnPercflow fichier trace = "
									+ fileTrace);
		
		
		
		outDurationMethods = new Trace(
				"####time"+sep+"class.method"+sep+"Value Return"+sep+"Length Return"+sep+"time called method in millisecondes\n",
				fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodValueReturnPercflow.durationMini"));
	}
	...
	after () returning (Object o): methods()	 {
		...
		if (duree/1000000 >= this.durationMini) {
			...
			outDurationMethods.append(new StringBuilder(outDurationMethods
					...
					sep).append(retour).append(sep).append(df.format(((double)duree/1000000))).append("\n")
					.toString());
			}
			else
			{
