...
public abstract aspect AbstractDurationSimpleMethodBeforeAfter {
	private static String fileTrace = "";

	public static WeakHashMap<String, Long> hmW = new WeakHashMap<String, Long>();
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodBeforeAfter.filelogs")) {
			fileTrace = dirLogs
					+ props.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodBeforeAfter.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out.println("AbstractDurationSimpleMethod14 fichier trace = "
				+ fileTrace);
		outDurationMethods = new Trace("####time" + sep + "Class.methods" + sep
				+ "time in millisecondes(ms)\n", fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodBeforeAfter.durationMini"));
	}
	...
	before(): methods()
	{
		...
		hmW.put(Thread.currentThread().getId() + "_" + thisJoinPoint.toString(),
				new Long(deb));
	}
	...
	after(): methods()	 {
		...
		// ne marche pas tout le temps car a la fin de la methode le thread
		// initial peut être ferme.
		// a retravailler
		long deb = hmW.get(key).longValue();
		...
		if (duree / 1000000 >= this.durationMini) {
			...
			if (noLine == 0) {
				outDurationMethods.append(new StringBuilder(outDurationMethods
						...
						.append(sep)
						.append(df.format(((double) duree / 1000000)))
						.append("\n").toString());
				...
				outDurationMethods.append(new StringBuilder(outDurationMethods
						...
						.append("_line").append(noLine).append(sep)
						.append(df.format(((double) duree / 1000000)))
						.append("\n").toString());
			}
		}
