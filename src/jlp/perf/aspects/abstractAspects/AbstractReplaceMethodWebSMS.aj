...
public privileged abstract aspect AbstractReplaceMethodWebSMS {
	...
	private static DecimalFormat dfPercent = new DecimalFormat("#0.0",
			new DecimalFormatSymbols(Locale.ENGLISH));
	private static String retour = "200";
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.concreteAspects.AbstractReplaceMethodWebSMS.filelogs")) {
			fileTrace = dirLogs
					+ props.getProperty("jlp.perf.aspects.concreteAspects.AbstractReplaceMethodWebSMS.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}
		retour = props
				.getProperty(
						"jlp.perf.aspects.concreteAspects.AbstractReplaceMethodWebSMS.retour",
						"200");
		traceEnabled = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.concreteAspects.AbstractReplaceMethodWebSMS.traceEnabled",
								"false"));

		if (traceEnabled) {
			System.out.println("AbstractReplaceMethodWebSMS fichier trace = "
					+ fileTrace);
			outDurationMethods = new Trace("####time" + sep
		...
		sleepTime = Double
				.parseDouble(props
						.getProperty(
								"jlp.perf.aspects.concreteAspects.ConcreteReplaceMethodWebSMS.sleepTime",
								"0"));
	}

	// Put it pointcut for Method to replace
	// pointcut methods(): call(String
	// com.btsl.channel.service.impl.PushMesssage.pushGatewayMessage(..));

	// public final pointcut methods(Runnable r): execution (void
	// myPackage.AThread.run(..)) && this(r);
	// public final pointcut methods(Runnable r): call (void
	// myPackage.AThread.run(..)) && target(r);
	// pointcut methods(): call(void myPackage.AThread.run(..));
	// public final pointcut methods(): execution(void
	// com.btsl.channel.service.impl.PushMesssage.run()) ;
