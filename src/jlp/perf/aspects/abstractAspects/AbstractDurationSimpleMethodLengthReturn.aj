...
public abstract aspect AbstractDurationSimpleMethodLengthReturn {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthReturn.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthReturn.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out
				.println("AbstractDurationSimpleMethodsLengthReturn fichier trace = "
						+ fileTrace);
		

		outDurationMethods = new Trace("####time"+sep+"class.method"+sep+"Length Return"+sep+"time called method in millisecondes(ms)\n", fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethodLengthReturn.durationMini"));
	}
	...
	Object around(): methods()	 {
		...
		if (duree/1000000 >= durationMini) {
			...
			outDurationMethods.append(new StringBuilder(outDurationMethods
					...
					sep).append(lenRet).append(sep).append(df.format(((double)duree/1000000))).append("\n")
					.toString());
