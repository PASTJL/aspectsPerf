...
public abstract aspect AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS {
	...
	public static String dirLogs,sep=";";
	static {
		...
		System.out.println("[Aspect Instrumentation AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS] Creation aspect AbstractPoolDBCPCountJDBCConnexionsMultiDS ");
		if (props.containsKey("jlp.perf.aspects.abstractAspects.AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}
			if (props.containsKey("jlp.perf.aspects.abstractAspects.AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.url")) {
			url = props
					.getProperty("jlp.perf.aspects.abstractAspects.AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.url");
		} 
		maxFreqLogs = Integer.parseInt(props
				.getProperty("jlp.perf.aspects.abstractAspects.AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.freqLogs","1"));
	...
	public final pointcut creationDatasource(): within(org.apache.tomcat.dbcp..*) && ( execution(public  org.apache.tomcat.dbcp.dbcp.BasicDataSource.new()) || execution(private * org.apache.tomcat.dbcp.dbcp.BasicDataSource.createDataSource(..))) ;
	...
	// Fermeture des connexions :
 	...
 	{
 		synchronized (AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS.this)
 		{
 	Object obj=proceed();
 	...
 	if (!trouve)
 	{
 		...
 		hmWeakStruc.put("DS_"+Integer.toString(countDS), swr);
 		 System.out.println( "[Aspect Instrumentation AbstractPoolTomcatDBCPCountJDBCConnexionsMultiDS] DS_"+Integer.toString(countDS)+" interceptee dans aspect");
 		countDS++;
 	}
 	tmpDsRef=null;
