...
public abstract aspect AbstractGenericAspect2 {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.perf.aspects.abstractAspects.AbstractGenericAspect2");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect2.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect2.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect2.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.abstractAspects.AbstractGenericAspect2.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect2.depthIntrospect",
						"All:-1;").split(";");
	...
	before(): methods()
	{
		...
		if (isTimeFreq) {
			Long lastTime = AbstractGenericAspect2.hmCurrentAge.get(nameClass);
			if ((System.currentTimeMillis() - lastTime) > (long) frequenceMeasure) {
				AbstractGenericAspect2.hmCurrentAge.put(nameClass,System.currentTimeMillis());
			} else {
				boolExec = false;
			}
			...
			if (lastTime >= (long) frequenceMeasure) {
				AbstractGenericAspect2.hmCurrentAge.put(nameClass,1L);
			} else {
				AbstractGenericAspect2.hmCurrentAge.put(nameClass,lastTime + 1);
				boolExec = false;
			}
		}
		...
		if (boolExec) {
			...
			Trace outFile=AbstractGenericAspect2.hmOutFiles.get(nameClass);
			if (hmBean.size()> 20)
				{
				hmBean.clear();
				counter=0;
