...
public abstract aspect AbstractDurationSimpleMethod {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethod.filelogs")) {
			fileTrace = dirLogs
					+ props.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethod.filelogs");
		} else {
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.abstractAspects.AbstractDurationSimpleMethod.durationMini"));
	}
		...
		if (duree / 1000000 >= durationMini) {
			...
			outDurationMethods
					.append(new StringBuilder(outDurationMethods.getSdf()
							...
							.append(sep)
							.append(thisJoinPointStaticPart.getSignature()
			...
			else
			{
				...
				.append(new StringBuilder(outDurationMethods.getSdf()
						...
						.append(thisJoinPointStaticPart.getSignature()
								.getDeclaringTypeName()).append(".")
						.append(thisJoinPointStaticPart.getSignature().getName())
						.append("_line_").append(noLine).append(sep)
						.append(df.format(((double) duree / 1000000)))
						.append("\n").toString());
			}
		}
