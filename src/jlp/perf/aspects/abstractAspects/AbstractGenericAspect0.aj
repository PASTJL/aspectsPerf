...
public abstract aspect AbstractGenericAspect0 {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.perf.aspects.abstractAspects.AbstractGenericAspect0");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect0.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect0.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect0.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.perf.aspects.abstractAspects.AbstractGenericAspect0.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.perf.aspects.abstractAspects.AbstractGenericAspect0.depthIntrospect",
						"All:-1;").split(";");
	...
	before(): methods()
	{
		...
		if (isTimeFreq) {
			Long lastTime = AbstractGenericAspect0.hmCurrentAge.get(nameClass);
			if ((System.currentTimeMillis() - lastTime) > (long) frequenceMeasure) {
				AbstractGenericAspect0.hmCurrentAge.put(nameClass,System.currentTimeMillis());
			} else {
				boolExec = false;
			}
			...
			if (lastTime >= (long) frequenceMeasure) {
				AbstractGenericAspect0.hmCurrentAge.put(nameClass,1L);
			} else {
				AbstractGenericAspect0.hmCurrentAge.put(nameClass,lastTime + 1);
				boolExec = false;
			}
		}
		...
		if (boolExec) {
			...
			Trace outFile=AbstractGenericAspect0.hmOutFiles.get(nameClass);
			if (hmBean.size()> 20)
				{
				hmBean.clear();
				counter=0;
