...
public privileged aspect ConcreteReplaceMethod {
	...
	static {
		...
		if (props.containsKey("jlp.perf.aspects.concreteAspects.ConcreteReplaceMethod.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.concreteAspects.ConcreteReplaceMethod.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}
		traceEnabled=Boolean.parseBoolean(props
		.getProperty("jlp.perf.aspects.concreteAspects.ConcreteReplaceMethod.traceEnabled","false"))
		;
    ...
    outDurationMethods = new Trace("####time"+sep+"Duration in millisecondes\n",fileTrace);
		...
		sleepTime = Double.parseDouble(props
				.getProperty("jlp.perf.aspects.concreteAspects.ConcreteReplaceMethod.sleepTime","0"));
	}

	// Put it pointcut for Method to replace
	public final pointcut methods(): execution(public * com.mmoney.sms.ConnectURL.run());
	...
	Object around(): methods()	 {
		...
		try {
		thisJoinPoint.getThis().getClass().getMethod("storeMessage",(Class[]) null).invoke(thisJoinPoint,(Object[])null);
		// invocation static method below
		Class.forName("com.mmoney.dbutilities.Database").getMethod("closeConnection", (Class[]) null).invoke(null,(Object[])null);
