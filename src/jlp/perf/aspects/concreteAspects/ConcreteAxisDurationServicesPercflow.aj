...
public aspect ConcreteAxisDurationServicesPercflow percflow(methods()) {
	...
	static {
		...
		if (props
				.containsKey("jlp.perf.aspects.concreteAspects.ConcreteAxisDurationServicesPercflow.filelogs")) {
			fileTrace = dirLogs
					+ props
							.getProperty("jlp.perf.aspects.concreteAspects.ConcreteAxisDurationServicesPercflow.filelogs");
		} else {
			fileTrace = props.getProperty("aspectsPerf.default.filelogs");
		}

		System.out.println("ConcreteAxisDurationServices fichier trace = "
				+ fileTrace);
		outDurationMethods = new Trace("####time" + sep + "Service.operation"
				+ sep + "time in millisecondes(ms)\n", fileTrace);
		/*
		...
		durationMini = Double
				.parseDouble(props
						.getProperty("jlp.perf.aspects.concreteAspects.ConcreteAxisDurationServicesPercflow.durationMini"));
	}

	public final pointcut methods(): execution(public * org.apache.axis.client.Call.invoke(..));
	...
	after(): methods()	 {
		...
		if (duree / 1000000 >= durationMini) {
			...
			if (null != thisJoinPointStaticPart.getSourceLocation()) {
				noLine = thisJoinPointStaticPart
			...
			try {
				...
				outDurationMethods.append(new StringBuilder(outDurationMethods
						...
						.append(sep).append(
								thisJoinPoint.getThis().getClass().getMethod(
										"getTargetEndpointAddress",(Class[]) null)
										.invoke(thisJoinPoint, (Object[]) null)).append(
								".").append(
								thisJoinPoint.getThis().getClass().getMethod(
										"getOperationName",(Class[])null).invoke(
										thisJoinPoint,(Object[]) null).toString())
						.append(sep).append(
					...
					outDurationMethods.append(new StringBuilder(outDurationMethods
							...
							.append(sep).append(
									...
									thisJoinPoint.getThis().getClass().getMethod(
											"getOperationName",(Class[])null).invoke(
											thisJoinPoint,(Object[]) null).toString())
							.append("_line").append(noLine).append(sep).append(
									df.format(duree / 1000000)).append("\n")
							.toString());
				}
				...
				System.out
						.println("AspectJ LTW => ConcreteAxisDurationServicesPercflow methods :"
								+ e2.getMessage());
