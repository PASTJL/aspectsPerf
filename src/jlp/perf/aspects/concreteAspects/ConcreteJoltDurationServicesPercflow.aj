...
public  aspect ConcreteJoltDurationServicesPercflow percflow(methods()) {
	...
	static {
		...
		if (props.containsKey("jlp.perf.aspects.concreteAspects.ConcreteJoltDurationServicesPercflow.filelogs")) {
			fileTrace = dirLogs+props
					.getProperty("jlp.perf.aspects.concreteAspects.ConcreteJoltDurationServicesPercflow.filelogs");
		} else {
    ...
    System.out.println("ConcreteJoltDurationServices fichier trace = "+fileTrace);
		...
		durationMini = Double.parseDouble(props
				.getProperty("jlp.perf.aspects.concreteAspects.ConcreteJoltDurationServicesPercflow.durationMini"));
	}
	...
	after (): methods()	 {
		...
		if (duree/1000000 >= this.durationMini) {
			...
			outDurationMethods
				...
				System.out
				.println("AspectJ LTW => ConcreteJoltDurationServicesPercflow methods :"
						+ e2.getMessage());
