...
public abstract aspect AbstractGenericMBean {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.aspectsJMX.abstractAspects.AbstractGenericMBean");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBean.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBean.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBean.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.aspectsJMX.abstractAspects.AbstractGenericMBean.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBean.depthIntrospect",
						"All:-1;").split(";");
		...
		try
		 {
			...
			if (hmBean.isEmpty() || !hmBean.containsKey(currentObject)) {
				...
				if (!hmInternalAttribute.containsKey(nameClass)
					...
					try {
						objName = new ObjectName("GenericMBean:type="
								+ shortName + ",name=" + name);
