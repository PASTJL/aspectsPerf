...
public abstract aspect AbstractGenericMBeanNumeric {
...
// Version for Java 1.5+
	...
	static {
		...
		System.out
				.println("Static initialisation Aspect : jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric");
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric.frequenceMeasure",
						"1");
		...
		String strNotWeave = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric.listNotWeave",
						"");
		...
		String strWeave = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric.listWeave",
						"");
		...
		isGetThis = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric.isGetThis",
								"true"));
		...
		String[] strDepth = props
				.getProperty(
						"jlp.aspectsJMX.abstractAspects.AbstractGenericMBeanNumeric.depthIntrospect",
						"All:-1;").split(";");
		...
		try
		 {
			...
			if (hmBean.isEmpty() || !hmBean.containsKey(currentObject)) {
				...
				if (!hmInternalAttribute.containsKey(nameClass)
					...
					try {
						objName = new ObjectName("GenericMBeanNumeric:type="
								+ shortName + ",name=" + name);
