...
public aspect ConcreteHttpSessionSizeBasic issingleton (){
	...
	static {
		...
		String strFreqLogs = props
				.getProperty(
						"jlp.aspectsJMX.concreteAspects.ConcreteHttpSessionSizeBasic.frequenceMeasure",
						"1");
		...
		serialization = Boolean
				.parseBoolean(props
						.getProperty(
								"jlp.aspectsJMX.concreteAspects.ConcreteHttpSessionSizeBasic.serialization",
								"false"));
	...
	public final pointcut methods(boolean bool): execution ( public * javax.servlet.http.HttpSession+.expire(boolean)) && args(bool) ;
	

	@SuppressWarnings("unchecked")
	before(boolean bool): methods(bool) 
	{
		...
		synchronized (this) {
			...
			try {
				name = new ObjectName(
						"AspectsConcrete:type=httpSessionSizeBasicInOctets_" + path);
			...
			if (freqCount >= frequenceMeasure) {
				...
				try {
					...
					System.out
							.println("AspectJ LTW => ConcreteHttpSessionSize methods2 :"
									+ e2.getMessage());
					return;
				} catch (SecurityException e2) {
					System.out
